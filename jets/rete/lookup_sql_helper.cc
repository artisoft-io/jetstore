  ...
  int LookupTable::lookup_internal(ReteSession * rete_session, bool is_multi, std::string const& key, RDFTTYPE * out)
  {
    ...
    try {      
      ...
      *out = *subject;
      ...
      if(rdf_session->contains(this->cache_uri_, lookup_row, subject)) return 0;
      ...
      while(not is_done) {
        ...
        for(int pos=0; pos<sz; pos++) {
          ...
          case rdf::rdf_named_resource_t   : value = rmgr->create_resource((char*)sqlite3_column_text(lc.stmt, pos)); break;
          ...
          case rdf::rdf_literal_string_t   : value = rmgr->create_literal((char*)sqlite3_column_text(lc.stmt, pos)); break;
          case rdf::rdf_literal_date_t     : value = rmgr->create_literal(rdf::parse_date((char*)sqlite3_column_text(lc.stmt, pos))); break;
          case rdf::rdf_literal_datetime_t : value = rmgr->create_literal(rdf::parse_datetime((char*)sqlite3_column_text(lc.stmt, pos))); break;
          default: {
