...
int create_jetstore_hdl(char const* rete_db_path,
                        char const* lookup_data_db_path, HJETS* handle) {
  if (not rete_db_path or not handle) return -1;
  ...
  } catch (const jets::rete_exception& ex) { // fix: catch by const reference
    LOG(ERROR) << "create_jetstore_hdl: ERROR while loading database '"
               << rete_db_path << "': " << ex;
    delete factory;
    *handle = nullptr;
    ...
    LOG(ERROR) << "create_jetstore_hdl: Unknown ERROR while loading database '"
               << rete_db_path << "'";
    delete factory;
    *handle = nullptr;
    return -1;
...
int create_rete_session(HJETS jets_hdl, HJRDF rdf_hdl, char const* jetrule_name,
                        HJRETE* handle) {
  ...
  if (not factory or not rdf_session) {
    LOG(ERROR) << "create_rete_session2: ERROR NULL factory for "
               << jetrule_name;
    return -1;
  }
  auto ms = factory->get_rete_meta_store(jetrule_name);
  if (not ms) {
    LOG(ERROR) << "::create_rete_session: ERROR ReteMetaStore not found for "
                  "main_rule file ";
    return -1;
  }
...
int create_meta_date_details(HJETS js_hdl, int year, int month, int day,
                             HJR* handle) {
  if (not js_hdl) return -1;
  auto* factory = static_cast<ReteMetaStoreFactory*>(js_hdl);
  if (not factory) {
  ...
  auto d = create_date(year, month, day);
  if (d.is_not_a_date()) return -2;
  *handle = factory->get_rmgr()->create_literal(d);
  return 0;
}
...
int create_meta_datetime_details(HJETS js_hdl, int year, int month, int day,
                                 int hr, int min, int sec, HJR* handle) {
  if (not js_hdl) return -1;
  auto* factory = static_cast<ReteMetaStoreFactory*>(js_hdl);
  auto d = create_datetime(year, month, day, hr, min, sec);
  if (d.is_not_a_date_time()) return -2;
  *handle = factory->get_rmgr()->create_literal(d);
  return 0;
}

int load_process_meta_triples(char const* jetrule_name, int is_rule_set,
                              HJETS js_hdl) {
  if (not jetrule_name) {
...
int insert_meta_graph(HJETS js_hdl, HJR s_hdl, HJR p_hdl, HJR o_hdl) {
  ...
  if (not mg) {
    LOG(ERROR)
        << "insert_meta_graph: while get_meta_graph: ERROR MetaGraph not found";
    return -1;
  }
...
int create_datetime_details(HJRDF hdl, int year, int month, int day, int hour,
                            int minute, int second, HJR* handle) {
  if (not hdl) return -1;
  auto* rdf_session = static_cast<RDFSession*>(hdl);
  auto d = create_datetime(year, month, day, hour, minute, second);
  if (d.is_not_a_date_time()) return -2;
  *handle = rdf_session->get_rmgr()->create_literal(d);
  return 0;
}
...
int get_resource_type(HJR handle) {
  ...
  switch (r->which()) {
    case rdf_null_t:
      return rdf_null_t;
    case rdf_blank_node_t:
      return rdf_blank_node_t;
    case rdf_named_resource_t:
      return rdf_named_resource_t;
    case rdf_literal_int32_t:
      return rdf_literal_int32_t;
    case rdf_literal_uint32_t:
      return rdf_literal_uint32_t;
    case rdf_literal_int64_t:
      return rdf_literal_int64_t;
    case rdf_literal_uint64_t:
      return rdf_literal_uint64_t;
    case rdf_literal_double_t:
      return rdf_literal_double_t;
    case rdf_literal_string_t:
      return rdf_literal_string_t;
    case rdf_literal_date_t:
      return rdf_literal_date_t;
    case rdf_literal_datetime_t:
      return rdf_literal_datetime_t;
    default:
      return -1;
  }
}
...
int get_resource_name(HJR handle, HSTR* v) {
  ...
  switch (r->which()) {
    case rdf_named_resource_t:
      *v = boost::get<NamedResource>(r)->name.data();
      return 0;
    default:
      return -1;
  }
...
char const* get_resource_name2(HJR handle, int* v) {
  ...
  switch (r->which()) {
    case rdf_named_resource_t:
      *v = 0;
      return boost::get<NamedResource>(r)->name.data();
    default:
      *v = -1;
      return nullptr;
  }
...
int get_datetime_details(HJR hdl, int* year, int* month, int* day, int* hr,
  ...
  switch (r->which()) {
    case rdf_literal_datetime_t: {
      ...
      *min = dur.minutes();
      *sec = dur.seconds();
      *frac = dur.fractional_seconds() *
              pow(10, 9 - time_duration::num_fractional_digits());
    }
      return 0;
    default:
      return -1;
...
int get_text_literal(HJR handle, HSTR* v) {
  ...
  switch (r->which()) {
    case rdf_literal_string_t:
      *v = boost::get<LString>(r)->data.data();
      return 0;
    default:
      return -1;
  }
...
char const* get_text_literal2(HJR handle, int* v) {
  ...
  switch (r->which()) {
    case rdf_literal_string_t:
      *v = 0;
      return boost::get<LString>(r)->data.data();
    default:
      *v = -1;
      return nullptr;
  }
...
int dump_rdf_graph(HJRDF hdl) {
  if (not hdl) return -1;
  auto* rdf_session = static_cast<RDFSession*>(hdl);
  std::cout << "Meta Graph Contains " << rdf_session->meta_graph()->size()
            << " triples (c++ version):" << std::endl;
  std::cout << rdf_session->meta_graph() << "---" << std::endl;
  std::cout << "Asserted Graph Contains "
            << rdf_session->asserted_graph()->size()
            << " triples (c++ version):" << std::endl;
  std::cout << rdf_session->asserted_graph() << "---" << std::endl;
  std::cout << "Inferred Graph Contains "
            << rdf_session->inferred_graph()->size()
            << " triples (c++ version):" << std::endl;
  std::cout << rdf_session->inferred_graph() << "---" << std::endl;
  std::cout << "The Meta Graph Contains:" << rdf_session->meta_graph()->size()
            << " triples" << std::endl;
  return 0;
}
...
// int find_asserted(HJRETE * rete_hdl, HJR * s, HJR * p, HJR * o, HJITERATOR **
// handle); int find_inferred(HJRETE * rete_hdl, HJR * s, HJR * p, HJR * o,
// HJITERATOR ** handle);
