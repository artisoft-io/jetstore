...
func TestMathMinMax1(t *testing.T) {
	...
	op := NewMinMaxOp(false, false)
	op.InitializeOperator(metaGraph, s, config)
	if !op.Eval(reteSession, nil, s, config).EQ(rdf.I(3)).Bool() {
