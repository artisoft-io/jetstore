...
// addRdfType: Add rdf type to the input ProcessInput.processInputMapping (ProcessMaps)
func (workspaceDb *WorkspaceDb) addRdfType(processMap []ProcessMap) error {
	for ipos := range processMap {
		dp := processMap[ipos].dataProperty
		if len(dp) > 3 && dp[0:4]=="_0:" {
			processMap[ipos].rdfType = "text"
		} else {
			err := workspaceDb.db.QueryRow(
				"SELECT type FROM data_properties WHERE name = ?", dp).Scan(&processMap[ipos].rdfType)
			if err != nil {
				return fmt.Errorf("while adding rdfType to ProcessMap for data_property %s: %v", processMap[ipos].dataProperty, err)
			}
		}
	}
	return nil
