...
func (ctx *BuilderContext) StartSplitterPipe(spec *PipeSpec, source *InputChannel, writePartitionsResultCh chan chan ComputePipesResult) {
	...
	for inRow := range source.channel {
		...
		if key == nil {
			log.Println(ctx.SessionId(), "*WARNING* splitter with nil key on source",source.config.Name)
		}
		...
		if splitCh == nil {
			...
			if ctx.cpConfig.ClusterConfig.IsDebugMode {
				if len(chanState) % 5 == 0 {
					log.Println(ctx.SessionId(), "splitter size:",len(chanState)," on source",source.config.Name)
				}
			}
