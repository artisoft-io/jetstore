...
date parse_date_internal(std::string&& text) {
  ...
  if (std::regex_search(text, what, internal::date_regex)) {
    int ntok = what.size();
    if (ntok < 4) {
      RDF_EXCEPTION("parse_date: Argument is not a date: " << text);
    }
    ...
    if (tok1.size() == 4) {
      // year, month, day
      return date(boost::lexical_cast<int>(tok1), month_2_int(tok2),
                  boost::lexical_cast<int>(tok3));
    } else {
      // month, day, year
      return date(boost::lexical_cast<int>(tok3), month_2_int(tok1),
...
datetime parse_time_internal(std::string&& text) {
  ...
  if (std::regex_search(text, what, internal::date_time_regex)) {
    int ntok = what.size();
    if (ntok < 4) {
      RDF_EXCEPTION("parse_date: Argument is not a datetime: " << text);
    }
    ...
    RDF_EXCEPTION("parse_time: Argument is not a datetime: " << text);
  }
}
