...
func main() {
	data, err := os.ReadFile("/home/michel/projects/repos/cedargate_ws/data/automated_mapping/unit_tests/file_encoding_utests/test2-iso-8859-1.csv")
	// data, err := os.ReadFile("/home/michel/projects/repos/cedargate_ws/data/automated_mapping/unit_tests/anonymize_dob/anonymize_unit_test - dob1.csv")
	check(err)
	detector := chardet.NewTextDetector()
	result, err := detector.DetectAll(data)
	var encoding string
	if err == nil {
		for i := range result {
	...
	case "UTF-8":
		...
		utfReader = transform.NewReader(inReader, unicode.BOMOverride(decoder))
	...
	default:
		log.Fatalf("Unsupported encoding: %s\n", encoding)
	}

	out, err := os.Create("outputFile.txt")
	if err != nil {
		log.Fatalf("Cant create output file: %v\n", err)
	}
	io.Copy(out, utfReader)
  out.Close()
	fmt.Println("Done!")
}
// // ==========================================================
...
// func NewISO8859_1Reader(r io.Reader) io.Reader {
// 	return charmap.ISO8859_1.NewDecoder().Reader(r)
// }

// func NewISO8859_2Reader(r io.Reader) io.Reader {
// 	return charmap.ISO8859_2.NewDecoder().Reader(r)
// }

// func NewWindows1250Reader(r io.Reader) io.Reader {
// 	return charmap.Windows1250.NewDecoder().Reader(r)
// }
