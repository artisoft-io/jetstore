...
func (ctx *Context) InsertSourcePeriod(year, month, day int) (int, error) {
	var key int64
	stmt := "SELECT key FROM jetsapi.source_period WHERE year=$1 AND month=$2 AND day=$3"
	err := ctx.dbpool.QueryRow(context.Background(), stmt, year, month, day).Scan(&key)
	switch {
	case err == nil:
		return int(key), nil
	case err.Error() == "no rows in result set":
		...
		err = ctx.dbpool.QueryRow(context.Background(), stmt).Scan(&key)
		if err != nil {
...
func (ctx *Context) RegisterFileKeys(registerFileKeyAction *RegisterFileKeyAction, token string) (*map[string]interface{}, int, error) {
	...
	for irow := range registerFileKeyAction.Data {
		...
		source_period_key, err := ctx.InsertSourcePeriod(
...
func (ctx *Context) StartPipelineOnInputRegistryInsert(registerFileKeyAction *RegisterFileKeyAction, token string) (*[]map[string]interface{}, int, error) {
	...
	for irow := range registerFileKeyAction.Data {
		...
		//*
		fmt.Println("pipeline_config keys that are ready to go:",*pipelineConfigKeys)
		...
		for _,pcKey := range *pipelineConfigKeys {
			...
			//*
			fmt.Printf("GOT corresponding merged_input_registry_keys: %v",merged_input_registry_keys)
...
func (ctx *Context) SyncFileKeys(registerFileKeyAction *RegisterFileKeyAction) (*map[string]interface{}, int, error) {
	...
	for s3Key := range s3Lookup {
		if !dbLookup[s3Key] {
			...
			source_period_key, err := ctx.InsertSourcePeriod(year, month, day)
			if err != nil {
				return nil, http.StatusInternalServerError, fmt.Errorf("while calling InsertSourcePeriod: %v", err)
			}	
