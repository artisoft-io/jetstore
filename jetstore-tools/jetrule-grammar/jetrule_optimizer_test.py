...
class JetRulesOptimizerTest(absltest.TestCase):
  ...
  def test_optimize2(self):
    ...
    optimized_data = jetrule_ctx.jetRules
    optimized_expected = """{"literals": [], "resources": [{"type": "resource", "id": "rdf:type", "value": "rdf:type"}, {"type": "resource", "id": "acme:Claim", "value": "acme:Claim"}, {"type": "volatile_resource", "id": "is_good", "value": "is_good"}], "lookup_tables": [], "jet_rules": [{"name": "RuleO2", "properties": {}, "antecedents": [{"type": "antecedent", "isNot": false, "triple": [{"type": "var", "value": "?clm02", "id": "?x1", "label": "?clm02"}, {"type": "identifier", "value": "rdf:type"}, {"type": "identifier", "value": "acme:Claim"}], "normalizedLabel": "(?x1 rdf:type acme:Claim)", "label": "(?clm02 rdf:type acme:Claim)"}, {"type": "antecedent", "isNot": false, "triple": [{"type": "var", "value": "?clm01", "id": "?x2", "label": "?clm01"}, {"type": "var", "value": "?good", "id": "?x3", "label": "?good"}, {"type": "keyword", "value": "true"}], "normalizedLabel": "(?x2 ?x3 true)", "label": "(?clm01 ?good true)"}], "consequents": [{"type": "consequent", "triple": [{"type": "var", "value": "?clm02", "id": "?x1", "label": "?clm02"}, {"type": "identifier", "value": "is_good"}, {"type": "keyword", "value": "true"}], "normalizedLabel": "(?x1 is_good true)", "label": "(?clm02 is_good true)"}], "authoredLabel": "[RuleO2]:(?clm01 ?good true).(?clm02 rdf:type acme:Claim) -> (?clm02 is_good true);", "normalizedLabel": "[RuleO2]:(?x1 rdf:type acme:Claim).(?x2 ?x3 true) -> (?x1 is_good true);", "label": "[RuleO2]:(?clm02 rdf:type acme:Claim).(?clm01 ?good true) -> (?clm02 is_good true);"}]}"""
    # print()
    # print('OPTIMIZED GOT:',json.dumps(optimized_data, indent=2))
    # print()
    # print('COMPACT:',json.dumps(optimized_data))
