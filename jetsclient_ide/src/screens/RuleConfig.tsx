/**
 * `/ruleConfig` — the rules configuration screen. Task C.10.
 *
 * ## Everything here is the v2 configuration, and the v1 siblings are gone
 *
 * C.0 deleted `ruleConfigTable`, `rulesConfigDialog` and `processConfigForm` with
 * the `/processConfig` route they belonged to. Nothing in this screen refers to
 * one, which was worth checking rather than assuming: the menu entry named
 * `processConfig` points at `ruleConfigPath`
 * (`jetsclient/lib/routes/jets_routes_app.dart`, `ruleConfigPath`), so the *name*
 * of the way in still says v1 and the destination has been v2 throughout.
 *
 * ## One dialog serving add and update, branched in the document
 *
 * `configureRulesv2` opens the same form whether or not a row is selected: with
 * one, five `navigationParams` seed it from the row and `key` is set; with none,
 * every field is empty. The action document posts `rule_configv2` under
 * `isNullOrEmpty key` and `update/rule_configv2` under its negation, which is the
 * shape `workspaceRegistry.ua.json` already uses for the same pairing.
 *
 * **`isReadOnlyWhenSet` is the piece that makes the branch safe.** The client and
 * the process identify the record on an update, so the Dart locks both once they
 * hold a value (`makeReadOnlyWhenHasSelectedValue`) — without it, editing either
 * on an existing row would move the record rather than edit it. It is a schema
 * member as of this task; see `userflow/form.ts`.
 *
 * ## One escape, and it is the first thing in track C the value grammar cannot say
 *
 * `process_config_key` is looked up in the rows the *process* dropdown's query
 * returned, by the chosen process name. `ValueSchema`'s five members all read form
 * state or a constant, so this is `resolveProcessConfigKey`
 * (`actions/ruleConfig.ts`), run as a step between `validate` and the two posts.
 *
 * ## The screen renders one table and declares no form document
 *
 * `rulesConfigv2SelectForm` is action-less and holds a single data-table field, so
 * this is I-177's clause for the third time in this session's three screens.
 */
...
import actionsJson from "./documents/ruleConfig.ua.json";
import formsJson from "./documents/ruleConfig.form.json";
import ruleConfigTableJson from "../datatable/tables/ruleConfigv2Table.tc.json";

/** The screen's own table, and the form-state key its selection publishes under. */
const TABLE_KEY = "ruleConfigv2Table";
...
/** The one table document. Bundled rather than read from a workspace (I-170). */
const TABLE_JSON: Record<string, unknown> = { ruleConfigv2Table: ruleConfigTableJson };
...
export function documentFindings(): string[] {
  ...
  const references = [
    ...formEscapeReferences(parsedForms.data as FormDocument, "ruleConfig"),
...
export function RuleConfig({ api }: { api: ApiClient }) {
  ...
  useEffect(() => {
    ...
    repeatRows.forEach((row, index) => {
      seed({ formState: dialogState, group: GROUP, flowKey: "ruleConfig" }, row, index);
    });
      ...
      void (async () => {
        ...
        try {
          switch (request.kind) {
            ...
            case "runAction": {
              // *Delete*. It confirms, posts the selected row's key, and the
              // table must re-read — a deletion that leaves the row on screen is
              // the failure that reads as working. **A refused confirmation is not
              // a deletion**, so it does not refresh either: re-reading after a
              // cancel is harmless and says the opposite of what happened.
              ...
              setStatus("Rule Configuration deleted");
            ...
            case "navigate": {
              ...
              // D.8 — see `UserAdmin.tsx` for why this is applied uniformly.
            ...
            case "escape": {
              ...
              await escape({ formState: activeState, group: GROUP, flowKey: "ruleConfig" }, host);
              return;
            }
            case "promptFilter":
  ...
  /** A dialog's button. `ruleConfigv2.ok` validates, resolves and posts; Cancel is the dialog's. */
